...
def pke_keygen():
    ...
    for _ in range(params.k):
        s.append(sampling.sample_poly_cbd(PRF(params.eta1, sigma, N)))
        N = N + 1

    #generate e
    e = []
    for _ in range(params.k):
        e.append(sampling.sample_poly_cbd(PRF(params.eta1, sigma, N)))
        N = N + 1
    ...
    t = []
    for row in range(params.k):
        t.append(As[row].poly_add(e[row]))
